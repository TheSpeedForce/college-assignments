...
if (isset($_POST['submit'])) {
    $typeId = fetch($con, "SELECT id FROM termType WHERE type='{$_POST['type']}'")[0];
    $query = "UPDATE term SET year = '{$_POST['year']}', termTypeId = '{$typeId['id']}' WHERE id = {$_POST['id']}";
    echo $query;
    fetch($con, $query);
}

if (isset($_POST['addNew'])) {
    $typeId = fetch($con, "SELECT id FROM termType WHERE type='{$_POST['addType']}'")[0];
    $query = "INSERT INTO term (year, termTypeId) VALUES ('{$_POST['addYear']}', '{$typeId['id']}')";
    fetch($con, $query);
}

if (isset($_POST['delete'])) {
    fetch($con, "DELETE FROM term WHERE id = {$_POST['delete']}");
}

$terms = fetch($con, "SELECT term.id, term.year, termType.type FROM term INNER JOIN termType ON termType.id=term.termTypeId");
...
<body>
                    ...
                    <li>
                        <div class="rvt-dropdown">
                            <button type="button" class="rvt-dropdown__toggle" data-dropdown-toggle="dropdown-2" aria-haspopup="true" aria-expanded="false">
                                <span class="rvt-dropdown__toggle-text"><a aria-current="page">Add New / Edit Exisiting Semester</a></span>
    ...
    <div class="container">
        <form action="editSemester.php" method="post">
            <main>
                <h1>Edit Terms</h1>
                <h3>Terms</h3>
                <table class="rvt-table-stripes">
                    ...
                    <tbody>
                        <?php
                        $termTypes = fetch($con, "SELECT type FROM termType");
                        foreach ($terms as $term) {
                            if ($edit == -1) {
                                echo "
                                <tr>
                                    <td scope='row'>{$term['id']}</td>
                                    <td scope='row'>{$term['type']}</td>
                                    <td scope='row'>{$term['year']}</td>
                                    <td class='addRow' hidden='true'></td>
                                    <td class='addRow' hidden='true'></td>
                                    <td scope='row'>
                                        <div class='rvt-button-group'>
                                            <form action='editSemester.php' method='post'>
                                                <button class='rvt-button' name='edit' value='{$term['id']}'>Edit</button>
                                            </form>
                                            </br >
                                            <form action='editSemester.php' method='post'>
                                                <button class='rvt-button rvt-button--danger' name='delete' value='{$term['id']}'>Delete</button>
                                            </form>
                                        </div>
                                    </td>
                                </tr>
                                ...
                                <tr>
                                    <form action='editSemester.php', method='post'>
                                        <td scope='row'>
                                            <input type='text' name='id' value='{$term['id']}' readonly>
                                        </td>
                                        <td><select name='type'>";
                                foreach ($termTypes as $s) {
                                    echo "<option >{$s['type']}</option>";
                                }
                                echo "</select></td>
                                        <td scope='row'>
                                            <input type='text' name='year' value='{$term['year']}'>
                                        </td>
                                        <td scope='row'>
                                            <form action='EditSemester.php' method='post'>
                                                <button class='rvt-button' name='submit' value='{$term['id']}'>Submit</button>
                                            </form>
                                        </td>
                                    </form>
                                </tr>
                        ...
                        <tr class='addRow' hidden='true'>
                            <form action='editSemester.php' method='post'>
                                ...
                                <td scope='row'>
                                    <input type='text' name='addYear' value='Year'>
                                </td>
                                <td scope='row'>
                                    <form action='editSemester.php' method='post'>
