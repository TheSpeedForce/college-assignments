...
if (isset($_POST['addNew'])) {
    ...
    fetch($con, "INSERT INTO supervisor (loginInfoId, roleId) VALUES ({$supId['id']}, 2)");
}

if (isset($_POST['delete'])) {
    $supId = $_POST['delete'];
    $loginInfoQuery = "SELECT loginInfoId as id FROM supervisor WHERE id={$supId}";
    $loginInfoId = fetch($con, $loginInfoQuery)[0]['id'];

    fetch($con, "DELETE FROM loginInfo WHERE id = {$loginInfoId}");
    fetch($con, "DELETE FROM supervisor WHERE id = {$adminId}");
}

$supervisors = fetch($con, "SELECT supervisor.id, loginInfo.username, loginInfo.email, loginInfo.fName, loginInfo.lName FROM supervisor INNER JOIN loginInfo ON loginInfo.id = supervisor.loginInfoId WHERE roleId = 2");
...
<head>
    ...
    <title>Edit Faculty Supervisor</title>
...
<header class="rvt-header" role="banner">
                ...
                <li>
                    <div class="rvt-dropdown">
                        <button type="button" class="rvt-dropdown__toggle" data-dropdown-toggle="dropdown-2" aria-haspopup="true" aria-expanded="false">
                            <span class="rvt-dropdown__toggle-text"><a aria-current="page">Add New / Edit Exisiting Faculty Supervisor</a></span>
    ...
    <div class="container">
        <form action="editFacultySupervisor.php" method="post">
            <main>
                <h1>Edit Faculty Supervisors</h1>
                    ...
                    <tbody>
                        ...
                        foreach ($supervisors as $sup) {
                            if ($edit == -1) {
                                ...
                                echo $sup['id'];
                                $students = fetch($con, "SELECT student.id, fName, lName, facultySupervisorId FROM `student` INNER JOIN loginInfo ON loginInfo.id = student.id WHERE student.facultySupervisorId = {$sup['id']}");
                                ...
                                echo "<td class='addRow' hidden='true'></td>
                                    ...
                                    <div class='rvt-button-group'>
                                        ...
                                        <form action='editFacultySupervisor.php' method='post'>
                                            <button class='rvt-button rvt-button--danger' name='delete' value='{$sup['id']}'>Delete</button>
                                        </form>
                                    </div>
                                </td>
                                    </td>
                        ...
                        <tr class='addRow' hidden='true'>
                            <form action='editFacultySupervisor.php' method='post'>
                                <td scope='row'></td>
                                <td scope='row'>
                                    <input type='text' name='addUserName' value='User Name' required='required'>
                                </td>
                                <td scope='row'>
                                    <input type='text' name='addEmail' value='Email' required='required'>
                                </td>
                                <td scope='row'>
                                    <input type='text' name='addFName' value='First' required='required'>
                                </td>
                                <td scope='row'>
                                    <input type='text' name='addLName' value='Last' required='required'>
                                </td>
                                <td>
                                ...
                                <td scope='row'>
                                    <input type='password' name='addPass' required='required'>
                                </td>
                                <td scope='row'>
                                    <input type='password' name='addPassConf' required='required'>
                                </td>
                                <td scope='row'>
                                    <form action='editFacultySupervisor.php' method='post'>
