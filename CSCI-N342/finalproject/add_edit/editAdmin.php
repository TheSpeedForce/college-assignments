...
if (isset($_POST['submit'])) {
    $adminId = fetch($con, "SELECT id FROM admin WHERE loginInfoId = {$_POST['submit']}")[0]['id'];
    echo $adminId;
    $query = "UPDATE loginInfo SET userName = '{$_POST['userName']}', email = '{$_POST['email']}', fName = '{$_POST['fName']}', lName = '{$_POST['lName']}' WHERE id = {$adminId}";
    fetch($con, $query);
}
...
if (isset($_POST['addNew'])) {
    ...
    $adminId = $con->lastInsertId();

    fetch($con, "INSERT INTO admin (loginInfoId) VALUES ({$adminId})");
}

if (isset($_POST['delete'])) {
    $adminId = $_POST['delete'];
    $loginInfoQuery = "SELECT loginInfoId as id FROM admin WHERE id={$adminId}";
    $loginInfoId = fetch($con, $loginInfoQuery)[0]['id'];

    fetch($con, "DELETE FROM loginInfo WHERE id = {$loginInfoId}");
    fetch($con, "DELETE FROM admin WHERE id = {$adminId}");
}

$admins = fetch($con, "SELECT admin.id, username, email, fName, lName FROM admin INNER JOIN loginInfo ON loginInfo.id=admin.loginInfoId");
?>
                        ...
                        foreach ($admins as $admin) {
                            if ($edit == -1) {
                                ...
                                <tr>
                                    ...
                                    <td scope='row'>{$admin['fName']}</td>
                                    <td scope='row'>{$admin['lName']}</td>
                                    <td class='addRow' hidden='true'></td>
                                    <td class='addRow' hidden='true'></td>
                                    <td scope='row'>
                                        <div class='rvt-button-group'>
                                            <form action='editAdmin.php' method='post'>
                                                <button class='rvt-button' name='edit' value='{$admin['id']}'>Edit</button>
                                            </form>
                                            </br >
                                            <form action='editAdmin.php' method='post'>
                                                <button class='rvt-button rvt-button--danger' name='delete' value='{$admin['id']}'>Delete</button>
                                            </form>
                                        </div>
                                    </td>
                                </tr>
                                ...
                                <tr>
                                    <form action='editAdmin.php', method='post'>
                                        ...
                                        <td scope='row'>
                                            <input type='text' name='fName' value='{$admin['fName']}'>
                                        </td>
                                        <td scope='row'>
                                            <input type='text' name='lName' value='{$admin['lName']}'>
                                        </td>
                                        <td class='addRow' hidden='true'></td>
                                        <td class='addRow' hidden='true'></td>
                                        <td scope='row'>
                                            <form action='editAdmin.php' method='post'>
                                                <button class='rvt-button' name='submit' value='{$admin['id']}'>Submit</button>
                                            </form>
                                        </td>
                                    </form>
                                </tr>
