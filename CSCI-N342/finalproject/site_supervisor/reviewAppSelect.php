...
<head>
    ...
    <title>Site Supervisor Menu</title>
</head>
...
<body>
    <header class="rvt-header" role="banner">
        ...
        <span class="rvt-header__title">
            <a href="#" class="inactiveLink">Site Supervisor Menu</a>
        </span>
        ...
        <div class="rvt-header__controls">
            ...
            <nav class="rvt-header__main-nav" role="navigation">
                <ul>
                    <li>
                        <a href="site_supervisorMenu.php">Dashboard</a>
                    </li>
                    <!-- First Navigation !-->
                    <li>
                        <a href="reviewApp.php" aria-current="page">Review Applications</a>
                    </li>
    ...
    <main role="main" id="main-content">
        <div class="rvt-container rvt-container--center rvt-container--junior rvt-m-bottom-xxl">
            <?php
            $query = "SELECT DISTINCT student.id, fName, lName FROM loginInfo 
            INNER JOIN student ON student.loginInfoId=loginInfo.id
            INNER JOIN hoursReport ON hoursReport.studentId=student.id
            WHERE hoursReport.siteApproval= 0;";

            $students = fetch($con, $query);
            $label = "Select student for approval";
            if ($students == NULL) {
                $label = "No students to approve";
            }
            ?>
            <select name="" id="select-info-state" class="rvt-validation-info" aria-describedby="inline-select">
                ...
                foreach($students as $student) {
                    echo "<option value='student-select'>{$student['lName']}, {$student['fName']}</option>";
                }
                ?>
            </select>
        </div>
