<?php
	...
	if(isset($_POST['submit_app'])){
		$submitApp = $_POST['submit_app'];
	}
	if($submitApp == 1){
		$query = 'INSERT INTO studentApplication (studentId, courseId, date, semesterId, pref1Id, pref2Id, pref3Id, considerations, resumeFile, studyPlanFile) VALUES ('.$_SESSION['POST'][0].',' .$_SESSION['POST'][1].', '.$_SESSION['POST'][2].', '.$_SESSION['POST'][3].', '.$_SESSION['POST'][4].', '.$_SESSION['POST'][5].', '.$_SESSION['POST'][6].', "'.$_SESSION['POST'][7].'", "'.$_SESSION['FILES'][0].'", "'.$_SESSION['FILES'][1].'")';
    ...
    <div class="rvt-header__controls">
        ...
        <nav class="rvt-header__main-nav" role="navigation">
            <ul>
                ...
                <!-- Second Navigation !-->
                <li>
                    <a href="hoursForm.php" >Reporting Hours</a>
                </li>
		...
		<div class="rvt-box__body">
			<ul class="rvt-inline-list">
				<li>
					...
					<label for="site1">Site Preference 1</label>
					<p id="site1">
					<?php
						foreach($siteRow as $i){
							if($_SESSION['POST'][4] == $i['id']){
								echo($i['name']);
							}
						}
					?>
					</p>
					<label for="site2">Site Preference 2</label>
					<p id="site2">
					<?php
						foreach($siteRow as $i){
							if($_SESSION['POST'][5] == $i['id']){
								echo($i['name']);
							}
						}
					?>
					</p>
					<label for="site3">Site Preference 3</label>
					<p id="site3">
					<?php
						foreach($siteRow as $i){
							if($_SESSION['POST'][6] == $i['id']){
								echo($i['name']);
							}
						}
					?>
					</p>
					<label for="consider"> Considerations</label>
					<p id="consider">
					<?php
						echo($_SESSION['POST'][7]);
					?>
					</p>
					<form action = "confirmIntern.php" method="post">
