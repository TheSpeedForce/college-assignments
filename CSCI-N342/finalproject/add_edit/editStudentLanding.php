...
if(isset($_POST['addStudent'])) {
    $q2 = "INSERT INTO loginInfo (fName, lName, username, email) VALUES ('{$_POST['updateFName']}', '{$_POST['updateLName']}', '{$_POST['updateUsername']}', '{$_POST['updateEmail']}')";
    fetch($con, $q2);
    $loginInfoId = $con->lastInsertId();
    $q3 = "INSERT INTO address (line1, line2, city, province, zip, country) VALUES ('{$_POST['updateLine1']}', '{$_POST['updateLine2']}', '{$_POST['updateCity']}', '{$_POST['updateProvince']}', {$_POST['updateZip']}, '{$_POST['updateCountry']}')";
    fetch($con, $q3);
    $addressId = $con->lastInsertId();
    $q1 = "INSERT INTO student (loginInfoId, addressId, advisorId, siteId, facultySupervisorId, siteSupervisorId, termId, phone)  VALUES ({$loginInfoId}, {$addressId}, {$_POST['updateAdvisor']}, {$_POST['updateSite']}, {$_POST['updateFSupervisor']}, {$_POST['updateSSupervisor']}, {$_POST['updateTerm']}, {$_POST['updatePhone']})";
    fetch($con, $q1);
}
$students = fetch($con, "SELECT student.id, loginInfo.fName, loginInfo.lName FROM student INNER JOIN loginInfo ON loginInfo.id=student.loginInfoId");
...
<header class="rvt-header" role="banner">
                ...
                <li>
                    <div class="rvt-dropdown">
                        <button type="button" class="rvt-dropdown__toggle" data-dropdown-toggle="dropdown-2" aria-haspopup="true" aria-expanded="false">
                            <span class="rvt-dropdown__toggle-text"><a aria-current="page">Add New / Edit Exisiting Student</a></span>
    ...
    <div class="container">
        <h1>Edit Students</h1>
        <label for="select-experience">Select Student:</label>

        <form action="editStudent.php" method="post">
            <select name="select-student">
                <?php
                foreach ($students as $student) {
                    echo "<option name='student' value='{$student['id']}'>{$student['fName']} {$student['lName']}</option>";
                }
                ?>
            </select><br>
            <button class='rvt-button' name='submit'>Submit</button>
            <button type="button" onclick="addStudent()" class='rvt-button' name='add'>+</button>
        </form>
        <form action="editStudentLanding.php" id="addStudent" hidden="true" method="post">
            ...
            <table>
                ...
                <tbody>
                    <td scope="row">
                        <input name="updateFName" type="text">
                    </td>
                    <td scope="row">
                        <input name="updateLName" type="text">
                    </td>
                    <td scope="row">
                        <input name="updateEmail" type="text">
                    </td>
                    <td scope="row">
                        <input name="updateUsername" type="text" va>
                    </td>
                    <td scope="row">
                        <input name="updatePhone" type="text">
                    </td>
                </tbody>
            </table>
            <br>
            <table>
                ...
                <tbody>
                    <td scope="row">
                        <input name="updateLine1" type="text">
                    </td>
                    <td scope="row">
                        <input name="updateLine2" type="text">
                    </td>
                    <td scope="row">
                        <input name="updateCity" type="text">
                    </td>
                    <td scope="row">
                        <input name="updateProvince" type="text">
                    </td>
                    <td scope="row">
                        <input name="updateZip" type="text">
                    </td>
                    <td scope="row">
                        <input name="updateCountry" type="text">
                    </td>
                </tbody>
            </table>
            <br>
            ...
            <button class='rvt-button' name='addStudent'>Add</button>
        </form>
    </div>
    <script>
        function addStudent() {
            var addStudent = document.getElementById('addStudent');
            addStudent.hidden = false;
        }
    </script>
