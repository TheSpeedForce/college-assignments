...
if (isset($_POST['submit'])) {
    $typeId = fetch($con, "SELECT id FROM siteType WHERE type='{$_POST['type']}'")[0];
    $query = "UPDATE site SET name = '{$_POST['name']}', type = '{$typeId['id']}' WHERE id = {$_POST['id']}";
    fetch($con, $query);
}

if (isset($_POST['addNew'])) {
    $typeId = fetch($con, "SELECT id FROM siteType WHERE type='{$_POST['addType']}'")[0];
    $query = "INSERT INTO site (name, type) VALUES ('{$_POST['addName']}', '{$typeId['id']}')";
    fetch($con, $query);
}
if (isset($_POST['delete'])) {
    fetch($con, "DELETE FROM site WHERE id = {$_POST['delete']}");
}

$sites = fetch($con, "SELECT site.id, site.name, siteType.type FROM site INNER JOIN siteType ON siteType.id=site.type");
                ...
                <li>
                    <div class="rvt-dropdown">
                        <button type="button" class="rvt-dropdown__toggle" data-dropdown-toggle="dropdown-2" aria-haspopup="true" aria-expanded="false">
                            <span class="rvt-dropdown__toggle-text"><a aria-current = "page">Add New / Edit Exisiting Site<a></span>
        ...
        <form action="editSite.php" method="post">
            <main>
                ...
                <table class="rvt-table-stripes">
                    ...
                    <tbody>
                        <?php
                        $siteTypes = fetch($con, "SELECT type FROM siteType");
                        foreach ($sites as $site) {
                            if ($edit == -1) {
                                ...
                                <tr>
                                    ...
                                    <td scope='row'>
                                        <div class='rvt-button-group'>
                                            <form action='editSite.php' method='post'>
                                                <button class='rvt-button' name='edit' value='{$site['id']}'>Edit</button>
                                            </form>
                                            </br >
                                            <form action='editSite.php' method='post'>
                                                <button class='rvt-button rvt-button--danger' name='delete' value='{$site['id']}'>Delete</button>
                                            </form>
                                        </div>
                                    </td>
                                </tr>
                                ...
                                <tr>
                                    <form action='editSite.php', method='post'>
                                        <td scope='row'>
                                            <input type='text' name='id' value='{$site['id']}' readonly>
                                        </td>
                                        <td scope='row'>
                                            <input type='text' name='name' value='{$site['name']}'>
                                        </td><td><select name='type'>";
                                        foreach($siteTypes as $s) {
                                            echo "<option >{$s['type']}</option>";
                                        }
                                        echo "</select></td><td scope='row'>
                                            <form action='editSite.php' method='post'>
                                                <button class='rvt-button' name='submit' value='{$site['id']}'>Submit</button>
                                            </form>
                                        </td>
                                    </form>
                                </tr>
